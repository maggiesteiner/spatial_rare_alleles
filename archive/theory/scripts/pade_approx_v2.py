...
def main():
    ...
    parser.add_argument("--filename", type=str, help="name of input file", default="results/spatial_integrals")
    ...
    for j in range(len(w_list)):
        ...
        for i in range(len(cols)): # 3 with u4
            coefs_w.append(data.loc[data["w"]==w,cols].values.tolist()[0][i]*(i+1))
        temp = calc_pade_table(coefs_w,args.calc_error)
        if args.calc_error==True:
            temp = calcError(temp,coefs_w)
        temp = calc_pole_res(temp)
        temp.insert(loc=0, column='w', value=np.repeat(w,temp.shape[0]))
