...
def main():
    ...
    w_list = [float(x) for x in data['w'].tolist()]
    ...
    if args.calc_error==True:
        ...
        df = pd.DataFrame(
            dict(w=w_list, residues=residues, poles=poles, poly_type=poly_type, pole_2=pole_2, res_2=res_2,
                 remainder=rem,error_next=error_next,rel_error=rel_error,total_deg=total_deg))
    else:
        df = pd.DataFrame(dict(w=w_list, residues=residues, poles=poles,poly_type=poly_type,pole_2=pole_2,res_2=res_2,remainder=rem))#,error_next=error_next,rel_error=rel_error
