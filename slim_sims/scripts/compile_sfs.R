...
for (file in file_list) {
  ...
  info <- strsplit(file_name, "_", fixed = TRUE)[[1]] %>% parse_number()
...
for (file_name in file_list) {
  ...
  params <- strsplit(file_name, "_")[[1]] %>% parse_number()
  ...
  num_samples <- as.integer(params[7])
  print(num_samples)  
...
df_summary <- df %>%
  ...
  group_by(s,width,`allele counts`,sampnum) %>% # mean across replicates
